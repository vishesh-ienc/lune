...
function listProcesses(forceRefresh = false) {
  const now = Date.now();
 ...
 */
function findRunningLanguageServerForProfile(profilePath) {
...
function safelyKillOrphanedProcess(pid) {
  ...
  try {
    ...
    console.log(`[Lune antigravity.js] Safely killed orphaned zombie language_server (PID: ${pid})`);
  } catch (_) {}
}
 ...
 */
function findAllActiveRunningLanguageServers() {
  const allProcs = listProcesses();
  const pidMap = new Map(allProcs.map(p => [p.pid, p]));
  ...
  for (const p of lspProcs) {
    if (isOrphanedLspProcess(p, pidMap)) {
      console.log(`[Lune watcher] PID ${p.pid} is an orphaned LSP (parent PID ${p.ppid} no longer running) — excluding from Live status.`);
    ...
    const ports = findListeningPorts(p.pid);
  ...
  //
  // 1. Exact language_server_windows_x64.exe process scan
  // 2. Direct parent inspection: Prefer LSP spawned directly by the main Antigravity GUI process
  //    ("C:\...\Antigravity IDE.exe" — command line does NOT contain --type=utility).
  // 3. Do NOT use PID, creation time, --workspace_id, or --enable_lsp as primary selection.

  if (results.length === 0) {
    console.log(`[LSP] No active LSP found`);
    return [];
  }
  ...
  if (results.length === 1) {
    ...
    console.log(`[LSP] Found 1 candidate Language Server process: PID ${r.pid} (Parent PID: ${r.ppid}, Parent Type: ${parentType})`);
    return results;
  }

  console.log(`[LSP] Found ${results.length} candidate Language Server processes\n`);
  for (const r of results) {
    const parentType = r.isMainGuiParent ? 'Main GUI' : 'Electron NodeService Utility';
    const priority = r.isMainGuiParent ? 'PRIMARY' : 'SECONDARY';
    console.log(`[LSP] PID ${r.pid}`);
    console.log(`      Parent: PID ${r.ppid}`);
  ...
  if (mainGuiCandidates.length === 1) {
    ...
    console.log(`[LSP] Selected Main-GUI LSP:`);
    console.log(`      PID ${selected.pid}\n`);
    return [selected];
  }

  if (mainGuiCandidates.length > 1) {
    console.log(`[LSP] Multiple main-GUI LSP candidates found (${mainGuiCandidates.length})`);
    console.log(`[LSP] Attempting identity resolution...`);
    return mainGuiCandidates;
  }

  // Fallback if no main-GUI LSP candidates found
  console.log(`[LSP] No Main-GUI LSP candidate found. Falling back to secondary candidates.`);
  return results;
}
...
function getAllListeningPortsMap(forceRefresh = false) {
  const now = Date.now();
...
function findListeningPorts(pid) {
  const map = getAllListeningPortsMap();
      ...
      res.on('end', () => {
        if (data && process.env.LUNE_DEBUG === 'true') {
          try {
            ...
            console.log(`[Lune Debug] Saved untruncated GetUserStatus JSON (${data.length} bytes) to ${outPath}`);
          } catch (e) {
...
function killProcessTree(topPid = null, userDataDir = null) {
  ...
  let currentProcs = listProcesses();
      ...
      if (IsWindowVisible(hWnd)) {
        ...
        if (len > 0) {
          ...
          if (
            ...
            console.log(`[Lune lib/antigravity] Closing OAuth browser window (hWnd: ${hWnd}, Title: "${title}")`);
        ...
        if (IsWindowVisible(hWnd)) {
          ...
          if (len > 0) {
            ...
            if (title.includes('Antigravity') || title.includes('Migrate') || title.includes('Import')) {
              console.log(`[Lune lib/antigravity] Dismissing modal on window (hWnd: ${hWnd}, Title: "${title}")`);
