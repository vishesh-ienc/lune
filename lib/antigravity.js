...
function findAllActiveRunningLanguageServers() {
  ...
  // Deduplicate per profile directory (userDataDir):
  // When switching accounts within the same IDE instance/profile, an older language_server process
  // may linger in the background alongside the newly spawned active session process.
  // For any given normalized userDataDir, keep ONLY the newest process (highest PID) as the active session.
  const newestByProfile = new Map();
  for (const item of results) {
    const normDir = path.normalize(item.userDataDir || '').toLowerCase();
    const existing = newestByProfile.get(normDir);

    if (!existing || item.pid > existing.pid) {
      if (existing) {
        console.log(`[Lune watcher] Multiple LSPs found for profile '${item.userDataDir}': PID ${existing.pid} (older) superseded by PID ${item.pid} (newest active session).`);
      }
      newestByProfile.set(normDir, item);
    } else {
      console.log(`[Lune watcher] Multiple LSPs found for profile '${item.userDataDir}': PID ${item.pid} (older) superseded by PID ${existing.pid} (newest active session).`);
    }
  }

  return Array.from(newestByProfile.values());
}
